...
[MRE_per,Var_per,sMRE_per,sVar_per] = Wang2010LNA.LNA(Theta_per, tspan, merr=0.0, Y0=MRE0, V0=Var0)
...
im = ax2.imshow(sVar[:, :, k1, k2, i])
ax2.set_title('analytical sensitivities')
fig.colorbar(im, ax=ax2)
im = ax3.imshow((Var_per[:, :, k1, k2] - Var[:, :, k1, k2]) / eps_theta - sVar[:, :, k1, k2, i])
ax3.set_title('error')
...
fig, (ax1, ax2, ax3) = plt.subplots(1, 3)
im = ax1.imshow((sVar_per[:, :, k1, k2, j] - sVar[:, :, k1, k2, j]) / eps_theta)
ax1.set_title('finite differences')
fig.colorbar(im, ax=ax1)
im = ax2.imshow(s2Var[:, :, i, j, k1, k2])
ax2.set_title('analytical sensitivities')
fig.colorbar(im, ax=ax2)
im = ax3.imshow((sVar_per[:, :, k1, k2, j] - sVar[:, :, k1, k2, j]) / eps_theta - s2Var[:, :, i, j, k1, k2])
ax3.set_title('error')
fig.colorbar(im, ax=ax3)
plt.show()
