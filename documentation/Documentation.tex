...
\subsubsection*{Blitz++}
Blitz++ version 0.9 or greater is required. It can be obtained via the included source code in \texttt{./libraries}, or from \href{https://sourceforge.net/projects/blitz/files/blitz/}{https://sourceforge.net/projects/blitz/files/blitz/}.  It is also available via macports on mac OS, and the Ubuntu 16.04 apt repository (libblitz0-dev, libblitz0v5). When using the provided source archive, simply extract it to ../libraries/blitz-0.10. For other versions of Blitz++ in non-default locations you need to adapt 'include_dirs' in 'python/setupTemplate.py'.

\subsubsection*{CVODES}
Numerical integration is computed using the CVODES Sundials package.  Source code is included in \texttt{./libraries}, and is also available from 
\href{http://computation.llnl.gov/casc/sundials}{http://computation.llnl.gov/casc/sundials}.

Source code for Blitz++ and CVODES can be installed via the standard GNU Autoconf toolchain (see \href{https://en.wikipedia.org/wiki/Autoconf}{here}) for further information. Usually it suffices to run the following commands from the source directory after unzipping:

\texttt{./configure --with-pic \\
make \\
sudo make install
}

This requires administrator privileges.  Otherwise, you can run

\texttt{make install --prefix=PATH/TO/INSTALL}
...
In order to use LNA++ with Python, you must first install the necessary scientific computing modules, listed below.  Python libraries can be installed using package managers such as apt on Ubuntu, or macports/homebrew on Mac OS, or using the \texttt{Pip3} Python package manager.
...
\texttt{sudo apt-get install libblitz0-dev libblitz0ldbl libblitz-doc python-sympy libsbml5-python}
...
\subsubsection*{(optional) pylab}
Pylab provides functions for plotting vectors and matrices, and might thus be useful.

\subsubsection*{Matlab}
In order to use LNA++ with Matlab, the Matlab Symbolic Toolbox is required for the analytic derivation of the LNA and its sensitivity equations. If Matlab or the Matlab Symbolic Toolbox is not available, LNA++ can still be used in two ways:
\begin{enumerate}
\item Python only.  Use the Python implementation (no Matlab) as described in the remainder of this document.
\item Mixed Matlab/Python.  Both the Python and Matlab implementations of LNA++ generate C code for the analytical components of 
\end{enumerate}
the LNA and its sensitivities.  It is therefore possible to generate the C code using the Python scripts, through the \texttt{generateLNAComponents} step, as described below. The Matlab executable is then generated in the normal way, resuming with the \texttt{compileLNA} step.
...
The stoichiometric matrix and reaction propensity functions are extracted from a user-supplied SBML model file, e.g., see \href{http://sbml.org/SBML_Software_Guide/SBML_Software_Summary}{http://sbml.org/}.

\subsection{Matlab}
To generate the Matlab executable simply call the function generateLNA specifying the path to the SBML input file, the model name and an optional argument indicating whether to attempt to compute the steady state of the system, see below.

It is recommended to run this command using the \texttt{models} subdirectory of the LNA++ root directory as the working directory.  This ensures the correct relative path for the subsequent build steps.

See \texttt{BirthDeath.m} in \texttt{./models} and the \textbf{Tutorial} for an example of how this is done.

\subsubsection*{Paths}
After running the functions for generating the mex file, the resulting C code, executable file, and wrapper function are placed in a subdirectory with the name of the model as specified.  Make sure that the wrapper function and executable are on the Matlab path, e.g. by changing to the directory of the model or by adding them to the path using \texttt{addpath}.

\subsubsection*{Building the model}
To build the model call \texttt{generateLNA(sbmlPath,model,computeSS)}
\begin{enumerate}
\item \texttt{sbmlPath}: the path to the SBML model specification file
\item \texttt{model}: the desired name of the model 
...
\begin{itemize}
\item \texttt{generateLNA(sbmlPath,model,computeSS,`Linker\_Flags',FLAGS)} or
\item \texttt{generateLNA(modelName,S,Npar,`Cpp\_flags',FLAGS)}.
\end{itemize}
...
\texttt{generateLNA} generates a mex file and a Matlab wrapper, which provides help messages and removes singleton dimensions for more convenient output. These are placed in the model subdirectory corresponding to the model specified.  To execute them, copy them to the desired final folder, or add them to the Matlab path. Usage of the generated function is specified in the help for that function.

See \texttt{BirthDeath} in \texttt{./models}, and generated \texttt{BirthDeath\_LNA.m} for an example of the generated wrapper function and its invocation.
...
\item \texttt{Time}: a vector of times at which the result of the model simulation is to be outputted. The first time is always zero.
...
\textit{Note:} It is recommended to save the model as a script in the \texttt{models} subdirectory of the LNA++ root directory.  This ensures the correct relative path for the subsequent build steps.

\subsubsection*{Creating the Python module}
After running generateLNA, a Python module is created and placed in the modules subdirectory. The module is named \texttt{modelLNA}, where model is replaced by the name of the model specified. The module can be imported using import, as long as the module is on the Python module path. To add this directory to the module path either modify the value of sys.path or add the modules directory to the \texttt{PYTHONPATH} environmental variable. 
...
\item \texttt{computeSens2}:  a Boolean variable indicating whether the second order sensitivities should be computed.  By default no second order sensitivities are computed.
\end{enumerate}
...
Note: if \texttt{computeSens2} is \texttt{True}, the first order sensitivities are always also computed and outputted.
...
It is possible to manually specify the components necessary for the computation of the linear noise approximation instead of deriving them from a supplied SBML file. For both Matlab and Python, the following components need to specified:
...
\subsubsection*{Matlab}
\begin{MatlabCode} 
% Define the stoichiometry matrix (S) for the model:
...
f = @(phi,t,Theta) [Theta(1),Theta(3)*phi(1),...
                    Theta(2)*phi(1),Theta(4)*phi(2)];

% Generate the symbolic C code:
addpath('../matlab')
generateLNAComponents('BirthDeath', S, f, phi, Theta, 'BOTH')
...
# Generate the symbolic C code:
tups = generateLNAComponents(model,S,fHandle,\
...
To test the performance of LNA++, we implemented a simple birth-death process involving a constitutively active DNA, and stochastic production and degradation of mRNA and protein (see Figure S1A, Table \ref{tab:BirthDeathReactions}).  The mRNA production reaction has a constant propensity $k_m$.  The propensity of production of a molecule of protein depends on the number of mRNAs, and on the rate constant $k_p$.  Lastly, mRNA is degraded with rate constant $\gamma_m$ and protein with rate constant $\gamma_p$.
...
\begin{figure}[htbp]
\begin{center}
\includegraphics{Figures/S1.pdf}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics{Figures/S3.pdf}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics{Figures/S5.pdf}
