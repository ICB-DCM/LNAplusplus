...
\end{eqnarray}
with $$(\bb{E} \bb{E}^T)'_k := \frac{d (\bb{E} \bb{E}^T)}{d \theta_k} = \frac{\partial (\bb{E} \bb{E}^T)}{\partial \bb{\phi}} \bb{\phi}'_k +  \frac{\partial (\bb{E} \bb{E}^T)}{\partial \theta_k}.$$ Note that \eqref{eq:Variance_sensitivity} and \eqref{eq:Variance_sensitivity 2} are alternative formulations. \eqref{eq:Variance_sensitivity 2} is more suited for numerical implementations as it avoids fractions with state variables (which might be zero) in the denominator.
