...
\subsubsection*{Blitz++}
Blitz++ version 1.0.1 is required. It can be obtained via the included source code in \texttt{./libraries}, or from \href{https://github.com/blitzpp/blitz}{https://github.com/blitzpp/blitz}.  It is also available via macports on mac OS, and the Ubuntu 16.04 apt repository (libblitz0-dev, libblitz0v5). 

When using the provided source archive, extract it to ../libraries/blitz-1.0.1, change to this folder and run:
\texttt{./configure --with-pic --prefix=`pwd`/install \\
make install
}

For other versions of Blitz++ in non-default locations you need to adapt 'include_dirs' and 'library_dirs' in 'python/setupTemplate.py'.
...
This requires administrator privileges.  Otherwise, you can run from within the cvodes directory:
