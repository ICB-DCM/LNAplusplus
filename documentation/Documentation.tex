...
Compilation of C/C++ source code for use as a standalone executable, or with Matlab or Python requires the use of a suitable compiler such as \texttt{gcc/g++} on Linux-based systems, or \texttt{clang} on mac OS.

\subsubsection*{Blitz++}
Blitz++ version 1.0.1 is required. Blitz++ 1.0.1 is included in the \texttt{./libraries} folder, or can be downloaded from \href{https://github.com/blitzpp/blitz}{https://github.com/blitzpp/blitz}. It is also available via macports on mac OS. 

When using the provided source archive, extract it to ../libraries/blitz-1.0.1, change to this folder and run:
\texttt{./configure --with-pic --prefix=`pwd`/install \&\& make install}

For other versions of Blitz++ in non-default locations you need to adapt \texttt{include\_dirs} and \texttt{library\_dirs} in
\texttt{python/setupTemplate.py}.
