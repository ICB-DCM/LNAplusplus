...
C_source = dir(['models/' model '/C/*.c']);
cmd_compile = [ sprintf(['mex -v -c -outdir models/%s/C ' ...
    '-Imodels/%s/C '], model, model), ...
    sprintf(['models/' model '/C/%s '], C_source.name)];
...
% 
cmd_compile_cpp = ['mex -c ../C++/computeLinearNoise.cpp ../C++/LNA_Onset.cpp -Imodels/' model '/C -I/usr/local/include'];
%disp(cmd_compile_cpp)
eval( cmd_compile_cpp )

%% link executable
disp('Linking')
Cobj = dir(['models/' model '/C/*.o']);
...
cmd_link = [ 'mex -cxx -output mex/' model '_LNA_mex  computeLinearNoise.o LNA_Onset.o '...
    sprintf(['models/' model '/C/%s '], Cobj.name) ... libstdcpp ...
